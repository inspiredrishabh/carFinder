...
const CarCard = ({ car }) => {
  // Hover effect ke liye state - mouse hover karne par effect ke liye
  const [isHovered, setIsHovered] = useState(false);
  
  return (
    <div 
      className="bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-700 h-full flex flex-col"
    ...
    >
      ...
      <div className="relative overflow-hidden h-52">
        ...
        {/* Car fuel type badge - fuel type info */}
        <div className="absolute top-4 left-4">
          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium shadow-lg 
            ${car.fuelType === 'Electric' ? 'bg-green-900 text-green-200' : 
            car.fuelType === 'Hybrid' ? 'bg-teal-900 text-teal-200' : 
            car.fuelType === 'Hydrogen' ? 'bg-blue-900 text-blue-200' : 
